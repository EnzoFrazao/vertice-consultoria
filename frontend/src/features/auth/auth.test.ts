...
import { DEMO_ADMIN_ID, DEMO_CLIENT_ID, DEMO_USERS } from "@/infrastructure/seed";
...
describe("autenticação demonstrativa", () => {
  it("autentica a conta cliente aprovada e cria sua sessão", () => {
    expect(authenticateDemoUser("cliente@demo.com", "cliente123", DEMO_USERS, () => NOW)).toEqual({
  ...
  it("autentica a conta administrativa aprovada", () => {
    expect(authenticateDemoUser("admin@demo.com", "admin123", DEMO_USERS, () => NOW)).toEqual({
  ...
  it("recusa senha incorreta, conta não demonstrativa e senha com espaços extras", () => {
    ...
    expect(
      authenticateDemoUser("fernanda@demo.com", "cliente123", DEMO_USERS, () => NOW)
    ).toBeNull();
    expect(
