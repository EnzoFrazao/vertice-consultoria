      ...
      <section className="overflow-hidden rounded-3xl border border-espresso/10 bg-ivory shadow-[0_18px_55px_rgba(32,19,13,0.08)]">
        <div className="bg-espresso px-5 py-6 text-ivory sm:px-7">
          ...
          <div className="mt-4 h-1.5 overflow-hidden rounded-full bg-white/10" aria-hidden="true">
            <div
              className="h-full rounded-full bg-bronze transition-[width] duration-300"
              style={{ width: `${step * 25}%` }}
            />
          </div>
            ...
            {steps.map(({ title, icon: Icon }, index) => {
              ...
              return (
                <li
                  ...
                  className={`flex items-center gap-2 rounded-xl px-3 py-2 text-xs font-semibold ${active ? "bg-white/10 text-ivory" : complete ? "text-champagne/80" : "text-ivory/40"}`}
                >
                  <span
                    className={`grid h-7 w-7 shrink-0 place-items-center rounded-lg ${active || complete ? "bg-bronze text-espresso" : "bg-white/10"}`}
                  >
                    {complete ? (
        ...
        <form onSubmit={handleSubmit} className="p-5 sm:p-7 lg:p-9">
          <h2
            ref={headingRef}
            tabIndex={-1}
            className="font-display text-2xl font-semibold text-espresso outline-none sm:text-3xl"
          >
            {steps[step - 1].title}
          </h2>
          ...
          {step === 1 ? (
            <div className="mt-6 grid gap-6">
              <div>
                ...
                {selectedService ? (
                  <p className="mt-2 text-sm leading-6 text-cacao/60">
                    {selectedService.description}
                  </p>
                ) : null}
              </div>
              <div>
                <label htmlFor="request-objective" className={labelClass}>
                ...
                />
                <p className="mt-2 text-xs leading-5 text-cacao/50">
                  Essa descrição ajuda a contextualizar a análise inicial.
                </p>
              </div>
            </div>
          ) : null}
          ...
          {step === 2 ? (
            <div className="mt-6 grid gap-5 sm:grid-cols-2">
              <div>
                ...
                <input id="request-name" value={currentUser.name} disabled className={fieldClass} />
              </div>
              <div>
              ...
              <div>
                <label htmlFor="request-cpf" className={labelClass}>
                  CPF
                </label>
                <input id="request-cpf" value={currentUser.cpf} disabled className={fieldClass} />
              </div>
              <div>
              ...
              <div className="sm:col-span-2">
                <label htmlFor="request-address" className={labelClass}>
                  Endereço
                </label>
                <input
                  id="request-address"
              ...
              <p className="sm:col-span-2 text-sm leading-6 text-cacao/60">
                E-mail e CPF identificam a conta demo e, por isso, permanecem bloqueados.
              </p>
            </div>
          ) : null}
          ...
          {step === 3 ? (
            <div className="mt-6 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
              ...
              <div>
                <label htmlFor="property-number" className={labelClass}>
                  Número
                </label>
                <input
                  id="property-number"
                  value={property.number}
                  onChange={(event) => updateProperty("number", event.target.value)}
                  className={fieldClass}
                />
              </div>
              <div>
                <label htmlFor="property-complement" className={labelClass}>
                  Complemento <span className="font-normal text-cacao/50">(opcional)</span>
                </label>
                <input
                  id="property-complement"
                  value={property.complement}
                  onChange={(event) => updateProperty("complement", event.target.value)}
                  className={fieldClass}
                />
              </div>
              <div>
                <label htmlFor="property-neighborhood" className={labelClass}>
                  Bairro
                </label>
                <input
                  id="property-neighborhood"
                  value={property.neighborhood}
                  onChange={(event) => updateProperty("neighborhood", event.target.value)}
                  className={fieldClass}
                />
              </div>
              <div>
              ...
              <div>
                <label htmlFor="property-state" className={labelClass}>
                  Estado
                </label>
                <input
                  id="property-state"
              ...
              <div>
                <label htmlFor="property-postal" className={labelClass}>
                  CEP
                </label>
                <input
                  id="property-postal"
              ...
              <div>
                <label htmlFor="property-registration" className={labelClass}>
                  Matrícula <span className="font-normal text-cacao/50">(opcional)</span>
                </label>
                <input
                  id="property-registration"
                  value={property.registrationNumber}
                  onChange={(event) => updateProperty("registrationNumber", event.target.value)}
                  className={fieldClass}
                />
              </div>
              <div>
                <label htmlFor="property-iptu" className={labelClass}>
                  Número do IPTU <span className="font-normal text-cacao/50">(opcional)</span>
                </label>
                <input
                  id="property-iptu"
                  value={property.iptuNumber}
                  onChange={(event) => updateProperty("iptuNumber", event.target.value)}
                  className={fieldClass}
                />
              </div>
              <div className="sm:col-span-2 lg:col-span-3">
                <label htmlFor="property-notes" className={labelClass}>
                  Observações <span className="font-normal text-cacao/50">(opcional)</span>
                </label>
                <textarea
                  id="property-notes"
                  rows={4}
            ...
            <div className="mt-6 grid gap-7 xl:grid-cols-[minmax(0,1fr)_20rem]">
              <div>
                <div className="rounded-2xl border border-tealTech/20 bg-tealTech/10 p-4 text-sm leading-6 text-tealTech">
                  <strong>Nenhum documento é obrigatório nesta demonstração.</strong> Os itens não
                  selecionados serão registrados como pendentes para envio posterior.
                </div>
                <fieldset className="mt-5 grid gap-3 sm:grid-cols-2">
                  ...
                  {state.mockDocumentAssets.map((asset) => {
                    ...
                    return (
                      <label
                        key={asset.id}
                        className={`flex min-h-28 cursor-pointer items-start gap-3 rounded-2xl border p-4 transition-colors ${selected ? "border-tealTech bg-tealTech/10" : "border-espresso/10 bg-white hover:border-tealTech/40"}`}
                      >
                        <input
                          type="checkbox"
                          checked={selected}
                        ...
                        <span className="min-w-0">
                          <span className="block font-semibold text-espresso">{asset.name}</span>
                          <span className="mt-1 block text-xs leading-5 text-cacao/60">
                            {asset.description}
                            <br />
              ...
              <aside className="h-fit rounded-3xl bg-espresso p-5 text-ivory">
                ...
                <dl className="mt-5 space-y-4 border-t border-white/10 pt-5 text-sm">
                  <div>
                    <dt className="text-xs uppercase tracking-wide text-ivory/40">Titular</dt>
                    <dd className="mt-1 text-champagne">{currentUser.name}</dd>
                  </div>
                  <div>
                    <dt className="text-xs uppercase tracking-wide text-ivory/40">Imóvel</dt>
                    <dd className="mt-1 leading-5 text-champagne">
                      {property.type} · {property.address}, {property.number}
                    </dd>
                  </div>
                  <div>
                    <dt className="text-xs uppercase tracking-wide text-ivory/40">
                      Exemplos selecionados
                    </dt>
                    <dd className="mt-1 text-champagne">
                      {selectedAssetIds.length} de {state.mockDocumentAssets.length}
                    </dd>
                  </div>
                </dl>
              </aside>
          ...
          {error ? (
            <p
              role="alert"
              className="mt-5 rounded-xl border border-red-200 bg-red-50 p-3 text-sm font-medium text-red-700"
            >
              {error}
            </p>
          ) : null}
          ...
          <div className="mt-8 flex flex-col-reverse gap-3 border-t border-espresso/10 pt-6 sm:flex-row sm:justify-between">
            {step > 1 ? (
              <button type="button" onClick={goBack} className={secondaryButtonClass}>
                <ChevronLeft aria-hidden="true" className="h-4 w-4" /> Voltar
              </button>
            ) : (
              <span />
            )}
            {step < 4 ? (
              <button type="button" onClick={goForward} className={primaryButtonClass}>
                Continuar <ChevronRight aria-hidden="true" className="h-4 w-4" />
              </button>
            ) : (
              <button type="submit" className={primaryButtonClass}>
                <FileCheck2 aria-hidden="true" className="h-4 w-4" /> Criar solicitação
              </button>
            )}
          </div>
        </form>
