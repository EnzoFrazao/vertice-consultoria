...
export function ServiceChoice({ onSelectService }: ServiceChoiceProps) {
  ...
  const activeCategory =
    SERVICE_CATEGORIES.find((category) => category.id === activeCategoryId) ??
  ...
  return (
    <section
      id="servicos"
      className="relative overflow-hidden bg-[linear-gradient(180deg,#fff9ef_0%,#f4e3cc_100%)] px-5 py-20 sm:px-8 sm:py-24 lg:px-10"
    >
        ...
        >
          {isDesktopExplorer ? (
            <div className="relative grid overflow-hidden rounded-xl border border-espresso/15 lg:grid-cols-[13rem_minmax(12rem,0.72fr)_minmax(18rem,1.28fr)]">
              <nav aria-label="Categorias de serviços" className="bg-espresso p-3 text-ivory">
                <p className="px-3 pb-3 pt-1 text-[11px] font-bold uppercase tracking-[0.2em] text-bronze">
                  Índice
                </p>
                    ...
                    return (
                      <li key={category.id}>
                        ...
                        >
                          <span
                            aria-hidden="true"
                            className="text-[10px] tracking-[0.16em] text-bronze"
                          >
              ...
              <section
                aria-labelledby="active-category-heading"
                className="border-r border-espresso/15 bg-ivory/75 p-4"
              >
                <p className="text-[11px] font-bold uppercase tracking-[0.18em] text-tealTech">
                  Categoria
                </p>
                <h3
                  id="active-category-heading"
                  className="mt-1 font-display text-xl font-semibold text-espresso"
                >
                  {activeCategory.name}
                </h3>
                    ...
                    return (
                      ...
                      >
                        ...
                        {selected ? (
                          <span
                            aria-hidden="true"
                            className="grid h-7 w-7 shrink-0 place-items-center border border-bronze bg-ivory text-tealTech"
                          >
              ...
              <section
                role="region"
                aria-label="Serviço em foco"
                className="relative flex min-h-[25rem] flex-col justify-between overflow-hidden bg-[linear-gradient(145deg,#fff9ef_0%,#f2dfc6_100%)] p-6"
              >
                <div className="relative">
                  <p className="text-[11px] font-bold uppercase tracking-[0.2em] text-tealTech">
                    Serviço selecionado
                  </p>
                  <h3 className="mt-6 max-w-xl font-display text-3xl font-semibold leading-tight text-espresso xl:text-4xl">
                    {activeService.name}
                  </h3>
                  <p className="mt-4 max-w-xl text-base leading-7 text-cacao/75">
                    {activeService.description}
                  </p>
                  <p className="mt-5 border-l-4 border-bronze pl-4 text-sm leading-6 text-cacao/70">
                    Ao entrar, sua nova solicitação já começa com este contexto — sem repetir a
                    escolha.
                  </p>
                </div>
                <button
                  ...
                  className="relative mt-8 inline-flex min-h-12 w-full items-center justify-center gap-3 rounded-lg bg-tealTech px-5 py-3 text-center font-bold text-white transition-colors hover:bg-[#0b625c] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-tealTech focus-visible:ring-offset-2"
                >
            ...
            <div className="relative divide-y divide-espresso/15 border-y border-espresso/15">
              {SERVICE_CATEGORIES.map((category, categoryIndex) => {
                ...
                return (
                  <details
                    key={category.id}
                    open={categoryIndex === 0 ? true : undefined}
                    className="group bg-ivory/70"
                  >
                    <summary className="flex min-h-16 cursor-pointer list-none items-center gap-3 px-3 py-4 marker:hidden focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-tealTech">
                      ...
                      <span className="min-w-0 flex-1">
                        ...
                        <span className="mt-1 block text-xs leading-5 text-cacao/70">
                          {category.description}
                        </span>
                      </span>
                      <ChevronRight
                        aria-hidden="true"
                        className="h-5 w-5 text-tealTech transition-transform group-open:rotate-90"
                      />
                    </summary>
                      ...
                      {selectedService ? (
                        <div className="mt-4 border-l-4 border-bronze bg-champagne/35 p-4">
                          ...
                          <button
                            ...
                            className="mt-4 inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-lg bg-tealTech px-4 py-3 text-center text-sm font-bold text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-tealTech focus-visible:ring-offset-2"
                          >
          ...
          <div className="relative mt-6 flex flex-col gap-4 rounded-xl bg-[linear-gradient(135deg,#20130d_0%,#0d2926_100%)] p-5 text-ivory sm:flex-row sm:items-center sm:justify-between sm:p-6">
            <div>
              ...
              <p className="mt-1 text-sm leading-6 text-champagne/75">
                Entre na sua conta para acompanhar documentos, pendências e histórico.
              </p>
            </div>
            <Link
              to="/login"
              className="inline-flex min-h-12 flex-none items-center justify-center gap-2 rounded-full border border-bronze/60 px-5 font-semibold text-champagne outline-none transition-colors hover:bg-white/10 focus-visible:ring-2 focus-visible:ring-bronze"
            >
              Acessar minha conta
