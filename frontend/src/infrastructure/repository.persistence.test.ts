...
import { DEMO_CLIENT_ID, DEMO_STATE_VERSION, createDemoSeed } from "@/infrastructure/seed";
import {
...
import type { StorageLike } from "@/infrastructure/repository";
...
describe("persistência do repositório demonstrativo", () => {
  it("inicializa e persiste o seed versionado quando o navegador ainda não tem dados", () => {
    ...
    expect(
      JSON.parse(localStorage.getItem(DEMO_STATE_STORAGE_KEY) ?? "{}").cases.length
    ).toBeGreaterThan(1);
