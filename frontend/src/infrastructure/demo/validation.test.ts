...
const invalidStateMutations: Array<[string, (state: DemoState) => void]> = [
  ...
  ["papel aninhado do usuário", (state) => void ((state.users[0] as { role: unknown }).role = "owner")],
  ["status do processo", (state) => void ((state.cases[0] as { status: unknown }).status = "Finalizado")],
  ["propriedade", (state) => void ((state.cases[0].property as { city: unknown }).city = 42)],
  ["status do documento", (state) => void ((state.cases[0].documents[0] as { status: unknown }).status = "Infectado")],
  ["versão do documento", (state) => void ((state.cases[0].documents[0].versions[0] as { submittedBy: unknown }).submittedBy = false)],
  ["evento da timeline", (state) => void ((state.cases[0].timeline[0] as { type: unknown }).type = "desconhecido")],
  ["notificação", (state) => void ((state.notifications[0] as { readAt: unknown }).readAt = 123)],
  ["asset de documento", (state) => void ((state.mockDocumentAssets[0] as { description: unknown }).description = null)]
];
...
const invalidDateMutations: Array<[string, (state: DemoState) => void]> = [
  ...
  ["revisão opcional do documento", (state) => void (state.cases[0].documents[0].reviewedAt = "nunca")],
  ["envio da versão", (state) => void (state.cases[0].documents[0].versions[0].submittedAt = "nunca")],
...
const invalidIntegrityMutations: Array<[string, (state: DemoState) => void]> = [
  ...
  ["id de propriedade duplicado", (state) => void (state.cases[1].property.id = state.cases[0].property.id)],
  ["id de documento duplicado", (state) => void (state.cases[1].documents[0].id = state.cases[0].documents[0].id)],
  ["id de versão duplicado", (state) => void (state.cases[0].documents[1].versions[0].id = state.cases[0].documents[0].versions[0].id)],
  ["id de evento duplicado", (state) => void (state.cases[0].timeline[1].id = state.cases[0].timeline[0].id)],
  ["id de notificação duplicado", (state) => void (state.notifications[1].id = state.notifications[0].id)],
  ["id de asset duplicado", (state) => void (state.mockDocumentAssets[1].id = state.mockDocumentAssets[0].id)],
  ...
  ["caseId incoerente na timeline", (state) => void (state.cases[0].timeline[0].caseId = "case-0002")],
  ["ator inexistente na timeline", (state) => void (state.cases[0].timeline[0].actorId = "user-ausente")],
  ["papel incoerente do ator na timeline", (state) => void (state.cases[0].timeline[0].actorRole = "admin")],
  ["asset inexistente na versão", (state) => void (state.cases[0].documents[0].versions[0].assetId = "asset-ausente")],
  ["asset incompatível com o tipo do documento", (state) => void (state.cases[0].documents[0].versions[0].assetId = "asset-iptu")],
  ["autor inexistente na versão", (state) => void (state.cases[0].documents[0].versions[0].submittedBy = "user-ausente")],
  ["revisor inexistente", (state) => void (state.cases[0].documents[0].reviewedBy = "user-ausente")],
  ["usuário inexistente na notificação", (state) => void (state.notifications[0].userId = "user-ausente")],
  ["processo inexistente na notificação", (state) => void (state.notifications[0].caseId = "case-ausente")],
  ["documento inexistente na notificação", (state) => {
...
describe("validação profunda dos dados demonstrativos", () => {
  ...
  it("restaura e persiste o seed quando um campo aninhado armazenado é inválido", () => {
    ...
    expect(isDemoState(JSON.parse(localStorage.getItem(DEMO_STATE_STORAGE_KEY) ?? "null"))).toBe(true);
  });
  ...
  it("restaura o seed quando o estado persistido tem referência inválida", () => {
    ...
    expect(repository.getState()).toEqual(createDemoSeed());
    expect(JSON.parse(localStorage.getItem(DEMO_STATE_STORAGE_KEY) ?? "null")).toEqual(createDemoSeed());
  });

  it("rejeita sessões malformadas e remove a sessão persistida", () => {
    const state = createDemoSeed();
    expect(isAuthSession({ userId: DEMO_CLIENT_ID, role: "owner", signedInAt: "agora" }, state)).toBe(false);
